...
//////end DOM creations
	...
	_defaultFilterJSON: function(json) {	//default callback for filter data
		...
		for(var i in json.result){
            num = '';
            if (json.result[i][propName].number) {num = json.result[i][propName].number+", ";}
            id = num+json.result[i][propName].street.name + " " +
                json.result[i][propName].street.postCode + " "+
                json.result[i][propName].street.municipality;
		    jsonret[ id ]= L.latLng(
                    json.result[i].point.y,
                    json.result[i].point.x);
        }
		return jsonret;
	},
